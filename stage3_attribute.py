...
PROMPT_VERSION = "tracked-object-single-pass-all-taxonomies-v14-local-context-batch"
WORDNET_SELECTION_VERSION = "candidate-id-v1"
...
def _resolve_wordnet_category(
    ...
    steps.append({
        ...
        "accepted_path": [root.name()],
    })
    return [root], steps, vlm_calls, warning
...
def _all_attributes_prompt() -> str:
    taxonomy_roots = _load_ontology()["attributes"]
    prompt_trees = {
        name: root for name, root in taxonomy_roots.items()
    }
    return f"""Classify the highlighted tracked object in ONE response.
...
Return:
1. A short common English object_name_hint.
2. Only the most specific visually supportable real WordNet noun synset ID, such as bottle.n.01. Stop at a broader valid synset instead of guessing. The application reconstructs its complete WordNet path locally.
3. Only one final-node ID for each of color, material, shape, and texture. Every ID must come from its corresponding controlled tree below. The selected ID may be an intermediate node when that is the most specific visually supportable choice. Use the explicit unknown child when evidence is insufficient. The application reconstructs all complete controlled paths locally.
...
{{
  "object_name_hint": "short common English noun or noun phrase",
  "category_synset": "real WordNet noun synset id",
  "attribute_ids": {{
...
No paths, markdown, reasons, or explanation."""
...
def _interpret_all_attributes(
    ...
    metadata = {
        ...
        "category_path": [_synset_option(node) for node in category_nodes],
...
def classify_all_attributes_single_pass(
    ...
    """Classify Category and all four controlled attributes in one VLM call."""
    answer, raw = _generate_json(
        images,
        _all_attributes_prompt(),
        model,
        processor,
        device,
        max_new_tokens=max_new_tokens,
    )
    category_resolution = _resolve_wordnet_category(
        images, answer, model, processor, device
    )
    return _interpret_all_attributes(answer, raw, category_resolution)
...
def classify_all_attributes_batch(
    ...
    results = []
    for images, raw in zip(image_groups, raws):
        try:
            answer = _extract_json(raw)
            category_resolution = _resolve_wordnet_category(
                images, answer, model, processor, device
            )
            results.append(_interpret_all_attributes(
                answer, raw, category_resolution
            ))
        except Exception as exc:
            results.append(exc)
    return results
...
def classify_wordnet_category(
    ...
    if not candidate_paths:
        ...
        return (
            ...
            root.name(),
            [_synset_option(root)],
            [{"decision_path": [], "accepted_path": [root.name()],
...
def classify_controlled_attributes(
    ...
    for _ in range(12):
        ...
        if not offered:
            break
        prompt = f"""Classify the highlighted tracked object in four controlled attribute trees.
The first image is the object isolated on black. The second image, when present, is the original scene with the same target highlighted. Use scene context to resolve ambiguity, but assign color, material, shape, and texture ONLY from the highlighted target; never copy properties from the table, robot, container, or other background objects.
...
Output ONLY JSON:
{{"decisions": {{"color": "listed id or STOP", "material": "listed id or STOP", "shape": "listed id or STOP", "texture": "listed id or STOP"}}, "reasons": {{"attribute": "short visual reason"}}}}
