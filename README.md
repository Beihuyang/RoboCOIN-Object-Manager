...
Stage 1 默认使用源视频第 0 帧发现物体。若单帧不能覆盖全部物体，可在人工审核页点击“追加关键帧”，拖动时间轴预览并确认；系统会在所选源帧重新运行 SAM3，同时保留此前关键帧及其物体。Stage 1 强制拆成独立阶段，默认跟踪抽帧率为 1 FPS：
...
自动发现不再只使用固定的 `object`。程序会从数据集目录名中去掉机器人型号、动作词、介词和编号，最多提取 6 个短名词或复合名词，例如 `move_test_tube` 会追加 `test tube` 和 `tube`，`storage_electronics_white_basket` 会追加 `white basket`、`electronics` 和 `basket`。先保留 `object` 结果，再加入语义提示发现的新掩码；跨提示掩码 IoU 达到 0.55，或包含率达到 0.80 且面积接近时视为重复。不同提示的置信度不直接比较，语义结果不会替换原来的 `object` 掩码。首帧发现与人工“确认并追加发现”使用同一组提示词。

自动发现阶段还会额外用 `robot arm`、`robot gripper`、`robot hand` 提示词检测机械臂区域；候选掩码有至少 50% 被这些区域覆盖时才会剔除。可用 `--robot-arm-threshold` 和 `--robot-arm-overlap` 调整，或用 `--no-exclude-robot-arms` 临时关闭。该机械臂重叠规则只用于自动发现，不用于人工框细化。
